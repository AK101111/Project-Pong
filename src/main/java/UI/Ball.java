        ...
        for(int i=0; i< Players.length; ++i) {
            switch (i){
                ...
                case 1:
                    ...
                    if(Players[i].getyCollisionBounds(1).left< ypos && ypos < Players[i].getyCollisionBounds(1).right && xpos+(radius)>=Players[i].getxpos() && !(Players[i].getdead())){
                        //System.out.println(String.format("Collide with %d Paddle %d %d",i+1,xpos,ypos));
                        return i;
                    }
                    break;
                case 2:
                    if(Players[i].getxCollisionBounds(-1).left< xpos && xpos < Players[i].getxCollisionBounds(-1).right && ypos+(radius)>=Players[i].getypos() && !(Players[i].getdead())){
                        //System.out.println(String.format("Collide with %d Paddle",i+1));
                        return i;
                    }
                    break;
